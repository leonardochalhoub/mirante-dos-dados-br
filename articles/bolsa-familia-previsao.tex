...
associação, \emph{identificamos seu efeito causal} por um instrumento
\emph{shift-share} (Bartik): o emprego formal reduz a demanda por
transferência, com efeito 2SLS de $-0{,}63$ (IC 95\% $[-1{,}20;-0{,}06]$,
primeiro estágio $F>26$), maior em magnitude que o de MQO \textemdash{}
sinal de atenuação. Em previsão, a autorregressão linear global supera não
só a persistência como os benchmarks clássicos ETS e SARIMA em todos os
horizontes (4,7\% vs.\ ${\sim}10\%$ de MAPE nacional em 12 meses). Conclui-se
...
erro de 4,7\% projeta com desvio da ordem de R\$\,7 bilhões/ano \textemdash{}
...
A leitura é que o emprego formal \emph{causa} redução da dependência do
Bolsa Família: criar vínculos formais retira famílias da elegibilidade,
não apenas se correlaciona com municípios que já têm menos pobreza. É a
contrapartida causal da leitura contracíclica da
Seção~\ref{sec:dados}, e dá fundamento estrutural \textemdash{} não apenas
preditivo \textemdash{} ao uso do emprego como covariável.
...
Para dimensionar o que esse erro significa em reais: a uma referência
recente de benefício médio da ordem de R\$\,700 por família, um erro
agregado de 4,7\% em doze meses sobre $\approx$\,18 milhões de famílias
corresponde a um desvio potencial da ordem de R\$\,0,6 bilhão por mês
\textemdash{} cerca de R\$\,7 bilhões ao ano \textemdash{} na parcela
estrutural do gasto projetado. É essa escala que torna a precisão da
projeção fiscalmente relevante, e que justifica buscar bandas calibradas
(Seção~\ref{sec:extensoes}). O cálculo é ilustrativo: o benefício médio é,
ele próprio, a alavanca de política, não uma constante.
