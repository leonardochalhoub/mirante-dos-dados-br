...
\begin{titlepage}
  \begin{center}
    ...
    {\small Working Paper \#10 \quad\textbullet\quad \textbf{Versão 1.0.1} \quad\textbullet\quad 23 de junho de 2026}\\[2pt]
    {\footnotesize Compilada em \COMPILESTAMP\ \textbullet\ \texttt{\COMPILESHA}}

    \vfill

    {\LARGE\bfseries
     CARGA DE CASOS, EMPREGO FORMAL E\\[6pt]
     CUSTO FISCAL DO PROGRAMA BOLSA FAMÍLIA}\\[14pt]

    {\large
      \textit{Previsão fora da amostra com redes neurais artificiais\\
      e identificação causal por instrumento shift-share\\
      (2013\textendash 2028)}}
...
relação estrutural entre cobertura do programa e emprego formal nos
municípios (correlação log-log de $-0{,}66$ em 2024) e, indo além da
associação, apresentamos \emph{evidência sugestiva} de seu efeito causal por
um instrumento \emph{shift-share} (Bartik): o emprego formal parece reduzir
a demanda por transferência, com efeito 2SLS de $-0{,}63$
(IC 95\% $[-1{,}20;-0{,}06]$, primeiro estágio $F>26$), maior em magnitude
que o de MQO, embora testes de falsificação recomendem cautela. Em previsão,
a autorregressão linear global supera não só a persistência como os
benchmarks clássicos ETS e SARIMA em todos os horizontes (4,7\% vs.\
${\sim}10\%$ de MAPE nacional em 12 meses). Deflacionando o gasto pelo IPCA,
mostramos que seu quase\-quadruplicar real (R\$\,41 $\to$ R\$\,144 bilhões de
2021, 2013\textendash 2024) foi conduzido pelo \emph{benefício médio real}
(R\$\,220 $\to$ R\$\,585 por família), não pela carga de casos. Conclui-se
que, sob ausência de novo choque institucional, a demanda estrutural é plana
ou declinante e que o custo futuro \textemdash{} cuja parcela estrutural um
erro de 4,7\% projeta com desvio da ordem de R\$\,6 bilhões/ano \textemdash{}
é governado pelo \emph{valor} do benefício, uma escolha de política, não por
uma carga de casos em expansão.

\medskip

\noindent\textbf{Palavras-chave}: identificação causal; instrumento
shift-share; variável instrumental; previsão de séries temporais; redes
...
backtest error below 4.7\% at twelve months, and beats classical ETS and
SARIMA benchmarks at every horizon (nationally and municipality by
municipality). We document a strong structural relationship between program
coverage and formal-employment intensity across municipalities (log-log
correlation $-0.66$ in 2024) and, beyond association, provide
\emph{suggestive} causal evidence from a shift-share (Bartik) instrument
that formal employment lowers program demand (2SLS effect $-0.63$, larger
than OLS), though placebo tests counsel caution. Deflating spending by the
IPCA, we show its near\-quadrupling in real terms (R\$\,41 to R\$\,144
billion of 2021, 2013\textendash 2024) was driven by the real average
benefit (R\$\,220 to R\$\,585), not the caseload. We conclude that
structural demand is flat-to-falling, so future cost is governed by the
benefit level \textemdash{} a policy choice.
...
decisão discricionária. É essa parcela que tratamos aqui. E adiantamos o
achado de política que dela decorre: deflacionado pelo IPCA, o gasto real do
programa quase \emph{quadruplicou} entre 2013 e 2024 (de R\$\,41 para
R\$\,144 bilhões de 2021), mas a carga de casos cresceu apenas cerca de
50\% \textemdash{} a explosão do custo veio do \emph{benefício médio real}
(que mais que dobrou), não da demanda (Seção~\ref{sec:projecao},
Figura~\ref{fig:gastoreal}). O custo do programa é, em grande medida, uma
escolha de política.
...
C++17 \textemdash{} inicialização de He, ReLU, retropropagação explícita,
...
das Estatísticas do Trabalho (PDET/MTE)~\C{ref:rais} \textemdash{} uma base
de \num{2060910061} vínculos formais (1985\textendash 2024) \textemdash{},
de onde extraímos o estoque municipal de vínculos ativos por ano.
...
RAIS \textendash{} PDET/MTE                 & vínculos formais & vínculo $\times$ ano & \num{2060910061} \\
\quad$\hookrightarrow$ painel derivado     & emprego formal/ano & município $\times$ ano & \num{83537} \\
IBGE                                       & população residente & município $\times$ ano & \num{66840} \\
\bottomrule
\end{tabular}
\par\smallskip
{\footnotesize Fontes oficiais. Camadas brutas: pagamentos PBF
(\num{2533885299}) e vínculos RAIS (\num{2060910061}, 1985\textendash 2024)
somam $\approx$\,4,6 bilhões de linhas. O painel PBF cobre 156 meses
(2013\textendash 2025); as covariáveis usadas, 2010\textendash 2024.}
\end{table}
...
Essa associação \emph{não} é mero artefato de escala. Controlando pela
população municipal, a correlação log-log permanece praticamente
inalterada ($-0{,}67$); e mesmo \emph{dentro} de cada unidade da federação
\textemdash{} comparando municípios do mesmo estado, o que absorve a
geografia histórica da pobreza por efeitos fixos \textemdash{}, a relação
persiste, com correlação de $-0{,}39$ e inclinação de $-0{,}36$ numa
regressão com efeitos fixos de UF ($R^2=0{,}71$); essa inclinação é
estatisticamente robusta a erro-padrão clusterizado por UF
($\text{EP}=0{,}064$, $t=-5{,}7$, 27 \emph{clusters}). Parte do $-0{,}66$
bruto reflete, portanto, o contraste \emph{entre} estados (Norte/Nordeste
vs.\ Sul/Sudeste), mas uma parcela substancial e robusta sobrevive
\emph{intra}-estado. Ainda assim, correlação não é causalidade: para
\emph{identificar} o efeito do emprego formal sobre a demanda por
transferência \textemdash{} isolando confundidores como nível de
desenvolvimento e composição setorial \textemdash{}, recorremos a um
instrumento \emph{shift-share} (Bartik), apresentado na
Seção~\ref{sec:causal}.
...
A relação, contudo, é \emph{estrutural} (de nível), não \emph{dinâmica}: ao
correlacionar as \emph{variações} de 2019 a 2024, a associação praticamente
desaparece (Figura~\ref{fig:crescimento}, $r\approx 0$). Isso é coerente
com o achado preditivo central \textemdash{} covariáveis de baixa
frequência informam o \emph{nível} de longo prazo da carga de casos, não
suas flutuações de curto prazo (Seção~\ref{sec:efeito}).
...
\caption{\textbf{Variação de caseload vs.\ emprego, 2019\textendash 2024.} A
correlação das \emph{variações} é praticamente nula ($r\approx 0$),
contrastando com a forte relação em \emph{nível} (Figura~\ref{fig:cob-emp},
$r=-0{,}66$): a associação é estrutural, não dinâmica.}
...
projeção; \emph{scripts} de análise computam os testes de significância e as
bandas (\texttt{analysis.py}), a identificação causal por Bartik
(\texttt{bartik\_iv.py}), os benchmarks clássicos, a banda conforme e os
testes de Mincer\-Zarnowitz e Diebold\-Mariano (\texttt{doctoral\_analysis.py});
um teste de unidade (\texttt{tests/test\_engine.cpp}) verifica o gradiente; e
o \emph{script} de figuras (\texttt{build-figures-pbf-forecast.py}) gera os
gráficos a partir dos artefatos versionados (\texttt{metrics.json},
\texttt{significance.json}, \texttt{doctoral\_stats.json},
\texttt{bartik\_iv.json}, \texttt{forward\_forecast.csv},
\texttt{forward\_band.csv}, \texttt{conformal\_band.csv},
\texttt{forecast\_h\{1,3,6,12\}.csv}, \texttt{errors\_h*.csv},
\texttt{training\_log.csv}, \texttt{covariates\_municipio\_ano.csv},
\texttt{muni\_year.csv}). O
pipeline é determinístico dados os geradores pseudoaleatórios semeados:
$\text{seed}=42$ na inicialização dos pesos (He) e $\text{seed}=7$ no
embaralhamento de minilotes e na separação do conjunto de validação. As
escolhas de desenho \textemdash{} janela $W=12$ (ciclo anual), arquitetura
$64\to 32$ (padrão da literatura, sem busca em grade) e paciência de cinco
épocas \textemdash{} foram fixadas \emph{a priori}, por razão de domínio,
antes de qualquer avaliação sobre 2025, de modo a não enviesar o desenho ao
conjunto de teste.

\medskip
\noindent\textbf{Declaração sobre ferramentas de IA.} A implementação do
código (o motor C++17, os \emph{scripts} de dados, de análise e de figuras)
e a redação deste artigo foram realizadas com assistência do modelo de
linguagem Claude Opus 4.8 (Anthropic), sob direção, verificação e revisão
dos autores. Essa assistência não compromete a integridade dos resultados,
que repousa em duas garantias verificáveis de forma independente: nenhum
número foi inventado \textemdash{} todos os valores são reproduzíveis a
partir dos artefatos de cômputo determinísticos descritos nesta seção
\textemdash{}, e toda afirmação conceitual está ancorada nas referências
oficiais listadas. Os autores assumem integral responsabilidade pelo
conteúdo.
...
\textemdash{} é implementado do zero em C++17, usando apenas
\texttt{<vector>}, \texttt{<cmath>}, \texttt{<map>} e \texttt{<random>}, sem
LibTorch, BLAS ou qualquer dependência de aprendizado profundo. Os pesos são armazenados em
...
custo de desempenho irrelevante nesta escala. A correção da retropropagação
manual é verificada por um teste de unidade que compara o gradiente
analítico ao numérico (diferenças finitas): o erro relativo máximo é de
$7{,}5\times 10^{-10}$, confirmando que cada derivada está corretamente
implementada.
...
\subsection{Comparação com o arsenal clássico (ETS e SARIMA)}

Um cético da literatura de previsão perguntaria: a autorregressão linear
global supera não apenas a persistência, mas os métodos estatísticos
clássicos? Para responder, ajustamos, sobre a série \emph{nacional} e no
mesmo protocolo fora do tempo de 2025, dois benchmarks consagrados: a
suavização exponencial de Holt\-Winters (ETS, com tendência e sazonalidade
aditivas) e um SARIMA $(1,1,1)(0,1,1)_{12}$. A
Figura~\ref{fig:benchmarks} mostra o resultado: a autorregressão linear do
estudo tem o \emph{menor} MAPE nacional em todos os horizontes, e a margem
\emph{cresce} com o horizonte \textemdash{} 4,7\% em 12 meses, contra
10,2\% do ETS e 10,1\% do SARIMA. Mais notável, ETS e SARIMA ficam
\emph{abaixo} da própria persistência em horizontes longos: ajustar
tendência e sazonalidade a uma série com quebras estruturais (os três
regimes do programa) produz extrapolações instáveis. O achado ecoa, mais
uma vez, a lição das competições M~\C{ref:m4}: sofisticação mal\-casada com
a estrutura do sinal piora, não melhora. Registre\-se, em honestidade, que
a comparação não é inteiramente \emph{ceteris paribus}: a autorregressão do
estudo é um modelo \emph{global}, treinado em \num{5542} séries de painel e
depois agregado, ao passo que ETS e SARIMA são univariados sobre a série
nacional. Parte da vantagem é, portanto, de \emph{dados} (a transferência de
aprendizado entre municípios), não só de método.

Para fechar essa ressalva, repetimos a comparação \emph{no nível em que a
inferência é feita} \textemdash{} município a município. Numa \emph{amostra
aleatória} de 400 municípios (semente fixa), ajustamos um ETS \emph{por município} e o comparamos, nas mesmas
previsões de seis meses, à autorregressão linear global. O resultado é ainda
mais favorável ao modelo do estudo: MAPE de \textbf{5,9\%} (AR linear global)
contra \textbf{8,4\%} do ETS local, 10,3\% da ingênua sazonal e 7,4\% da
persistência \textemdash{} o ETS local fica, aliás, \emph{abaixo} da própria
persistência, repetindo o padrão nacional. Confirma\-se assim, agora
\emph{ceteris paribus}, o achado mais forte: um modelo \emph{global},
treinado no painel de \num{5542} séries, supera métodos clássicos univariados
aplicados série a série \textemdash{} é a transferência de aprendizado entre
municípios, não a arquitetura, que entrega o ganho.

\begin{figure}[H]
\centering
\includegraphics[width=0.95\textwidth]{fig19_benchmarks.pdf}
\caption{\textbf{Benchmarks clássicos.} MAPE da série nacional (2025): a
autorregressão linear global do estudo supera ETS e SARIMA em todos os
horizontes, e a vantagem cresce com o horizonte.}
\label{fig:benchmarks}
\end{figure}

Dois testes formais complementam o quadro, ambos sobre a série nacional.
O teste de Mincer\-Zarnowitz não rejeita a hipótese de não\-viés a um mês
(estatística de Wald $=0{,}28$), mas a \emph{rejeita} nos horizontes longos
(Wald $=7{,}0$, $23{,}6$ e $6{,}6$ em 3, 6 e 12 meses, acima do crítico
$\chi^2_2 = 5{,}99$) \textemdash{} sinal de viés agregado da projeção recursiva
nesses horizontes. Como a baixa potência de 12 observações torna a rejeição
\emph{conservadora} (mais difícil rejeitar), o sinal é genuíno; reforça,
portanto, que o nível adequado de inferência é o painel municipal, não o
agregado, e que a projeção nacional de longo prazo deve ser lida com a
banda de incerteza, não como ponto. O teste de
Diebold\-Mariano~\C{ref:diebold} entre o modelo linear e a persistência, com
correção de pequena amostra de Harvey\-Leybourne\-Newbold, fica abaixo do
limiar de significância nos horizontes curtos e é \emph{numericamente
instável} em 12 meses (a variância de longo prazo do diferencial colapsa com
$n=12$ e forte autocorrelação) \textemdash{} ambos reflexo direto das 12
observações. Já o bootstrap pareado sobre as \num{66504} previsões
municipais (Seção anterior) tem potência ampla. É a razão de centrarmos a
inferência no painel municipal, não no agregado: $n=12$ não sustenta um
...
% =====================================================================
\section{Identificação causal: o emprego formal reduz a demanda por transferência}
\label{sec:causal}
% =====================================================================

As seções anteriores estabeleceram que o emprego formal \emph{prevê} a
carga de casos (covariáveis ajudam no longo prazo, Seção~\ref{sec:efeito})
e que cobertura e emprego se \emph{associam} negativamente entre municípios
(Seção~\ref{sec:dados}). Resta a pergunta causal: o emprego formal
\emph{reduz} a demanda por Bolsa Família, ou ambos apenas refletem um
terceiro fator (o nível de desenvolvimento local)? Esta seção a enfrenta com
um desenho de identificação \emph{shift-share}.

\subsection{O instrumento de Bartik}

A endogeneidade é clara: municípios mais pobres têm, simultaneamente, menos
emprego formal e mais Bolsa Família, de modo que a regressão simples confunde
causa e seleção. A estratégia de \emph{shift-share}, ou de
Bartik~\C{ref:bartik}, resolve isso construindo um preditor do crescimento
do emprego que não depende das condições locais de demanda. Para o município
$c$, definimos
\begin{equation}
B_c \;=\; \sum_{s} \omega_{c,s,t_0}\;g_{s,-c}^{\,t_0\to t_1},
\label{eq:bartik}
\end{equation}
onde $\omega_{c,s,t_0}$ é a \emph{participação} do setor $s$ no emprego
formal do município no ano-base $t_0$ (2013), e $g_{s,-c}$ é o crescimento
\emph{nacional} do setor $s$ entre $t_0$ e $t_1$, calculado \emph{excluindo}
o próprio município (\emph{leave-one-out}). A intuição: um município
especializado em setores que cresceram nacionalmente herda esse crescimento
por composição, e essa parcela do crescimento do emprego é plausivelmente
exógena às condições locais de pobreza. A identificação repousa na
exogeneidade das participações setoriais iniciais condicional aos controles,
no sentido de Goldsmith-Pinkham, Sorkin e Swift~\C{ref:gpss}; com 92 setores
CNAE, o instrumento agrega muitos choques setoriais~\C{ref:bhj}.

Estimamos, em primeiras diferenças de longo prazo,
\begin{equation}
\Delta\log\!\big(\text{cobertura}_c\big)
= \beta\,\Delta\log\!\big(\text{emprego}_c\big) + \bm\gamma'\bm w_c + \varepsilon_c,
\label{eq:2sls}
\end{equation}
instrumentando $\Delta\log(\text{emprego}_c)$ por $B_c$, com controles
$\bm w_c$ (efeitos fixos de UF e log da população\-base) e erro\-padrão
clusterizado por UF. Reportamos duas janelas: 2013\textendash 2019 (regime
PBF estável, antes do choque do Auxílio Brasil) e 2013\textendash 2024
(completa).

\subsection{Primeiro estágio e efeito causal}

O instrumento é forte. Na janela limpa 2013\textendash 2019, o primeiro
estágio tem coeficiente $\pi=+1{,}06$ e estatística $F=67{,}7$
(Figura~\ref{fig:firststage}) \textemdash{} muito acima do limiar usual de
10; na janela completa, $F=26{,}3$. Não há, portanto, problema de
instrumento fraco.

\begin{figure}[H]
\centering
\includegraphics[width=0.95\textwidth]{fig18_bartik_first_stage.pdf}
\caption{\textbf{Primeiro estágio do shift-share.} O instrumento de Bartik
(participação setorial inicial $\times$ crescimento nacional) prevê
fortemente o crescimento efetivo do emprego formal municipal,
2013\textendash 2019 ($F=67{,}7$).}
\label{fig:firststage}
\end{figure}

A Tabela~\ref{tab:iv} e a Figura~\ref{fig:iv} trazem o resultado central.
O efeito causal estimado por variável instrumental (2SLS) é \emph{negativo}
nas duas janelas e \emph{maior em magnitude} que o de MQO \textemdash{}
$-0{,}31$ vs.\ $-0{,}05$ (2013\textendash 2019) e $-0{,}63$ vs.\ $-0{,}06$
(2013\textendash 2024). Esse padrão \textemdash{} IV mais negativo que MQO
\textemdash{} é a assinatura de \emph{atenuação}: a regressão simples,
contaminada por erro de medida e endogeneidade, \emph{subestima} o efeito
do emprego. Na janela completa o efeito é estatisticamente significativo
($\beta_{\text{IV}}=-0{,}63$, $t=-2{,}2$, IC 95\% $[-1{,}20;-0{,}06]$); na
janela pré\-choque, o sinal e a magnitude são os mesmos, porém o intervalo
cruza zero \textemdash{} mesma evidência, menor potência no regime estável,
de menor variação de emprego.

\begin{table}[H]
\centering
\caption{Efeito do emprego formal sobre a cobertura do PBF (2SLS, EP clusterizado por UF)}
\label{tab:iv}
\small
\begin{tabular}{@{}lcccc@{}}
\toprule
\textbf{Janela} & \textbf{MQO $\beta$} & \textbf{IV $\beta$} & \textbf{IC 95\% (IV)} & \textbf{$F$ 1\textsuperscript{o} est.} \\
\midrule
2013\textendash 2019 (pré-choque) & $-0{,}05$ & $-0{,}31$ & $[-0{,}77;\,+0{,}15]$ & 67,7 \\
2013\textendash 2024 (completa)   & $-0{,}06$ & $\bm{-0{,}63}$ & $[-1{,}20;\,-0{,}06]$ & 26,3 \\
\bottomrule
\end{tabular}
\par\smallskip
{\footnotesize $n=\num{5211}$ municípios, 27 \emph{clusters} (UF). Controles:
efeitos fixos de UF e log-população. IV \emph{just-identified} pelo
instrumento de Bartik. Fonte: elaboração própria sobre RAIS, CGU e IBGE.}
\end{table}

\begin{figure}[H]
\centering
\includegraphics[width=0.95\textwidth]{fig20_causal_iv.pdf}
\caption{\textbf{Efeito causal vs.\ MQO.} O efeito 2SLS do emprego formal
sobre a cobertura do PBF é negativo e maior em magnitude que o de MQO
(atenuação), significativo na janela completa.}
\label{fig:iv}
\end{figure}

\subsection{Quais setores identificam o efeito? Pesos de Rotemberg}

Um instrumento de Bartik é uma média ponderada de estimadores setoriais; os
pesos de Rotemberg~\C{ref:gpss} revelam quais setores CNAE dominam a
identificação \textemdash{} e, portanto, se a exogeneidade é plausível. A
Tabela~\ref{tab:rotemberg} lista os setores de maior peso. Na janela limpa
(2013\textendash 2019), o instrumento é dominado por manufatura
\emph{transacionável} \textemdash{} confecção de vestuário, couro/calçados,
obras de infraestrutura \textemdash{}, setores cuja sorte responde à demanda
nacional, e não às condições locais de pobreza: exatamente o tipo de choque
que a hipótese de exogeneidade requer.

\begin{table}[H]
\centering
\caption{Setores de maior peso de Rotemberg no instrumento de Bartik}
\label{tab:rotemberg}
\small
\begin{tabular}{@{}lr@{\hskip 2.2em}lr@{}}
\toprule
\multicolumn{2}{c}{\textbf{2013\textendash 2019 (limpa)}} &
\multicolumn{2}{c}{\textbf{2013\textendash 2024 (completa)}} \\
\textbf{Setor} & \textbf{$\alpha$} & \textbf{Setor} & \textbf{$\alpha$} \\
\midrule
Confecção de vestuário   & $+0{,}24$ & Couro e calçados      & $+0{,}66$ \\
Obras de infraestrutura  & $+0{,}21$ & Alimentos             & $+0{,}29$ \\
Couro e calçados         & $+0{,}15$ & Têxteis               & $+0{,}28$ \\
Comércio varejista       & $-0{,}13$ & Comércio atacadista   & $-0{,}24$ \\
Coque e derivados        & $+0{,}11$ & Seleção de mão de obra & $+0{,}23$ \\
\bottomrule
\end{tabular}
\par\smallskip
{\footnotesize Pesos de Rotemberg normalizados (somam 1). Fonte: elaboração
própria sobre a RAIS setorial (92 setores CNAE).}
\end{table}

Esses pesos também explicam a inversão de sinal do primeiro estágio na
janela completa. Ali o instrumento passa a ser dominado por manufatura
\emph{trabalho\-intensiva} (couro/calçados, têxteis, alimentos)
\textemdash{} setores que cresceram \emph{nacionalmente} na recuperação
pós\-2015 e pós\-pandemia, mas \emph{encolheram localmente} onde eram
concentrados, por desindustrialização e competição de importados. A
previsão shift\-share (participações de 2013 $\times$ crescimento nacional)
inverte\-se, portanto, não por falha do instrumento, mas porque o conteúdo
econômico do choque mudou entre os regimes. A exogeneidade \textemdash{}
choques setoriais nacionais, não condições locais de pobreza \textemdash{}
permanece; o sinal do primeiro estágio é que reflete a mecânica setorial.

\subsection{Testes de falsificação (placebo)}

A exogeneidade do instrumento exige testes de placebo, e os reportamos com
transparência \textemdash{} inclusive quando incômodos. Dois exames. No
\emph{teste de balanço}, regredimos o \emph{nível} de cobertura de 2013
(pré\-período) sobre o instrumento: o coeficiente é positivo e marginalmente
significativo ($\beta=+2{,}76$, $t=+2{,}3$). No \emph{teste de
pré\-tendência}, regredimos o crescimento do emprego de 2009 a 2013
(anterior à janela de choque) sobre o instrumento: o coeficiente é grande e
significativo ($\beta=-0{,}96$, $t=-8{,}6$). Nenhum dos dois é nulo, como o
ideal pediria.

Há, porém, uma leitura mecânica importante: o instrumento usa
\emph{participações de 2013} \textemdash{} o fim do pré\-período
\textemdash{}, e participações medidas ao fim de uma janela são, por
construção, correlacionadas com o crescimento \emph{dentro} daquela janela
(2009\textendash 2013), o que infla o teste de pré\-tendência; a literatura
recomenda, por isso, participações de um ano\-base bem anterior~\C{ref:bhj}.
A correlação com o nível de 2013, por sua vez, reflete que a composição
setorial se associa à estrutura econômica local, que por sua vez se associa
à pobreza \textemdash{} o desenho em primeiras diferenças \emph{diferencia}
esse nível, mas não podemos testar pré\-tendências no \emph{resultado}
porque o painel PBF começa em 2013. O líquido honesto: os placebos
\emph{não} validam de forma limpa a restrição de exclusão, e por isso
rebaixamos a leitura de ``evidência causal moderada'' para \emph{evidência
sugestiva}, consistente com um canal causal, mas a ser confirmada por um
desenho com ano\-base anterior a 2009 ou por uma descontinuidade de
elegibilidade (Seção~\ref{sec:extensoes}).

\subsection{Interpretação e cautelas}

A leitura sugerida \textemdash{} com as ressalvas dos placebos \textemdash{}
é que o emprego formal \emph{reduz} a dependência do Bolsa Família: criar
vínculos formais retira famílias da elegibilidade, não apenas se correlaciona
com municípios que já têm menos pobreza. É a contrapartida causal plausível
da leitura contracíclica da Seção~\ref{sec:dados}, e dá fundamento
estrutural \textemdash{} não apenas preditivo \textemdash{} ao uso do emprego
como covariável.

Três cautelas. \emph{Primeira}: a significância vem da janela completa, cujo
primeiro estágio inverte de sinal por mudança na composição setorial do
instrumento (subseção anterior), não por instrumento fraco ($F=26{,}3$);
ainda assim, tratamos a estimativa pontual com a correspondente cautela.
\emph{Segunda}: a exogeneidade do shift-share é uma
hipótese de identificação, não um fato \textemdash{} ela falha se as
participações setoriais de 2013 já anteciparem trajetórias de pobreza.
\emph{Terceira}: especificações sem efeitos fixos de UF, que exploram
variação nacional, são imprecisas demais para inferência. Por isso
descrevemos o achado como \emph{evidência sugestiva}, consistente com um
canal causal mas não suficiente para afirmá\-lo \textemdash{} a
identificação \emph{causal} permanece como agenda aberta, a ser fechada por
um desenho com ano\-base anterior a 2009, uma descontinuidade de
elegibilidade ou um \emph{event study} de fechamentos de planta via CAGED
(Seção~\ref{sec:extensoes}).
...
futuro. Note\-se que, aqui, usar toda a história (inclusive 2025) para
ajustar o modelo e padronizar as séries \emph{não} é vazamento: a projeção
prevê 2026\textendash 2028, meses genuinamente não observados, e no momento
real da previsão toda a série até dezembro de 2025 está, de fato,
disponível. O vazamento é evitado onde importa \textemdash{} na
\emph{avaliação} fora do tempo (Seção~\ref{sec:protocolo}), cuja
padronização usa exclusivamente meses anteriores ao corte, e de onde vêm
todos os números de acurácia e a inferência reportados. Somamos as previsões municipais à trajetória nacional
...
torno de 15,4 milhões de famílias até 2028. A banda sombreada é um intervalo
\emph{informal}, derivado do MAPE agregado de backtest de 2025 \textemdash{}
não um intervalo de confiança frequentista de 95\%.}
\label{fig:serie}
\end{figure}
...
Banda informal em dez/2028                          & $[\,\num{13999098};\ \num{16886844}\,]$ \\
\bottomrule
\end{tabular}
\par\smallskip
{\footnotesize Banda \emph{informal} de $\pm$MAPE de backtest (não um IC de
95\%), de 1,3\% (curto) a 9,4\% (dez/2028). Como o backtest cobre um único
ano (2025) e a recursão amplifica o erro (equação~\ref{eq:recursao}), a
banda deve ser lida como aproximação conservadora, não como cobertura
calibrada. Fonte: elaboração própria.}
\end{table}

\subsection{Banda calibrada por predição conforme}

A banda informal pode ser substituída por uma com \emph{garantia de
cobertura}. Aplicamos \emph{predição conforme} dividida~\C{ref:conformal}:
tomamos os erros relativos de \emph{backtest} de 2025 como escores de
não\-conformidade e, para cada horizonte, usamos o quantil empírico
$\lceil(n+1)(1-\alpha)\rceil/n$ como meia\-largura do intervalo. Sob
permutabilidade, o intervalo resultante tem cobertura marginal de pelo menos
$1-\alpha$. A Figura~\ref{fig:conformal} compara as duas bandas: a conforme
(90\%) é da mesma ordem de grandeza da informal \textemdash{} 3,4\% a 9,2\%
de meia\-largura relativa, de 1 a 12 meses \textemdash{}, o que valida a
banda informal como aproximação razoável, agora com uma alternativa
calibrada disponível para uso orçamentário. A pequena amostra de calibração
(12 meses de 2025 por horizonte) torna os quantis conformes granulares,
limitação que séries de backtest mais longas reduziriam.\footnote{Com
$n=12$ escores por horizonte e $\alpha=0{,}10$, o quantil conforme usável é
o 11.\textsuperscript{o} da amostra ordenada, de modo que a cobertura
marginal \emph{efetiva} é $11/13 \approx 84{,}6\%$, e não exatamente 90\%;
para uso orçamentário, a banda deve ser lida com essa cobertura efetiva.}
Como a banda é calibrada sobre os mesmos erros de backtest que o teste de
Mincer\-Zarnowitz apontou enviesados em horizontes longos, ela herda esse
viés de projeção recursiva: é, portanto, conservadora quanto à variância,
mas não corrige o viés sistemático \textemdash{} razão adicional para
tratar a projeção de longo prazo como cenário, não como ponto.

\begin{figure}[H]
\centering
\includegraphics[width=0.92\textwidth]{fig21_conformal.pdf}
\caption{\textbf{Banda conforme vs.\ informal.} Meia\-largura relativa do
intervalo nacional por horizonte: a predição conforme (90\%, com garantia
de cobertura) é próxima da banda informal de $\pm$MAPE.}
\label{fig:conformal}
\end{figure}
...
A decomposição custo $=$ carga de casos $\times$ benefício
(equação~\ref{eq:custo}) ganha conteúdo quando deflacionamos o gasto a
\emph{reais de 2021} pelo IPCA (BCB). A Figura~\ref{fig:gastoreal} e a
Tabela~\ref{tab:gastoreal} mostram o ponto central deste artigo em sua
forma mais nítida: entre 2013 e 2024 o gasto real do programa quase
\emph{quadruplicou} \textemdash{} de R\$\,41 para R\$\,144 bilhões de 2021
\textemdash{}, mas a carga de casos cresceu apenas cerca de 50\%. A
diferença é o \emph{benefício médio real}, que saltou de
$\approx$\,R\$\,220 para $\approx$\,R\$\,585 por família\-mês. A explosão
do custo foi conduzida pela \emph{alavanca de política}, não pela demanda
estrutural \textemdash{} exatamente o que a equação~\eqref{eq:custo}
prediz e o que a projeção plana da carga de casos confirma para o futuro.

\begin{figure}[H]
\centering
\includegraphics[width=0.95\textwidth]{fig22_gasto_real.pdf}
\caption{\textbf{O gasto explodiu pelo benefício, não pela carga de
casos.} Índices (2013 = 100) da carga de casos, do benefício médio real e
do gasto real (R\$ de 2021, deflator IPCA): o gasto acompanha o benefício,
não a contagem de famílias.}
\label{fig:gastoreal}
\end{figure}

\begin{table}[H]
\centering
\caption{Gasto real e benefício médio (R\$ de 2021, deflator IPCA)}
\label{tab:gastoreal}
\small
\begin{tabular}{@{}lrrr@{}}
\toprule
\textbf{Ano} & \textbf{Gasto real (R\$ bi)} & \textbf{Benefício médio (R\$/fam.)} & \textbf{Carga (mi)} \\
\midrule
2013 & 41,0 & 248 & 14,0 \\
2019 & 36,6 & 221 & 13,1 \\
2021 & 38,6 & 221 & 14,1 \\
...
\end{tabular}
\par\smallskip
{\footnotesize Valores deflacionados a R\$ de dezembro de 2021 pelo IPCA
(BCB, série 433). Fonte: elaboração própria sobre CGU, IBGE, BCB.}
\end{table}

Isso também torna rigorosa a tradução do erro de previsão em reais. Ao
benefício médio real de 2024 (R\$\,585 por família\-mês), um erro agregado
de 4,7\% em doze meses sobre $\approx$\,18 milhões de famílias corresponde
a um desvio da ordem de R\$\,0,5 bilhão por mês \textemdash{} cerca de
R\$\,6 bilhões ao ano \textemdash{} na parcela estrutural do gasto
projetado. É essa escala que torna a precisão da projeção fiscalmente
relevante.
...
cresce \textemdash{} sob a hipótese, explicitada nas limitações, de ausência
de novo choque institucional ou de elegibilidade.
...
\textbf{Sexta:} a relação emprego$\leftrightarrow$PBF é \emph{consistente
com causalidade}, não apenas associativa, com as ressalvas da
Seção~\ref{sec:causal}. Um instrumento \emph{shift-share} (Bartik) identifica
um efeito 2SLS negativo do emprego formal sobre a cobertura do programa,
significativo na janela completa ($-0{,}63$, $t=-2{,}2$) e maior que o de
MQO \textemdash{} indício de que criar emprego formal \emph{retira} famílias
da elegibilidade. E, em previsão, a autorregressão global \emph{bate o
arsenal clássico} (ETS, SARIMA) em todos os horizontes.

\textbf{Sétima:} o \emph{custo fiscal é uma escolha de política}, não uma
...
peça explicitamente em C++17, seguindo Chen e Gupta~\C{ref:chen-gupta}, e ao
...
\refentry{ref:bartik}
BARTIK, Timothy J. \textbf{Who Benefits from State and Local Economic
Development Policies?} Kalamazoo: W. E. Upjohn Institute for Employment
Research, 1991. DOI: 10.17848/9780585223940.

\refentry{ref:gpss}
GOLDSMITH-PINKHAM, Paul; SORKIN, Isaac; SWIFT, Henry. Bartik instruments:
what, when, why, and how. \textbf{American Economic Review}, v. 110, n. 8,
p. 2586\textendash 2624, 2020. DOI: 10.1257/aer.20181047.

\refentry{ref:bhj}
BORUSYAK, Kirill; HULL, Peter; JARAVEL, Xavier. Quasi-experimental
shift-share research designs. \textbf{The Review of Economic Studies},
v. 89, n. 1, p. 181\textendash 213, 2022. DOI: 10.1093/restud/rdab030.
