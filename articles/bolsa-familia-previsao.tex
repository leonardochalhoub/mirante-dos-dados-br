...
formal nos municípios (correlação log-log de $-0{,}66$ em 2024, robusta a
efeitos fixos de estado e ao controle por população, porém associativa, não
causal). Conclui-se que, sob ausência de novo choque institucional, a
demanda estrutural é plana ou declinante e que o custo futuro é governado
pelo \emph{valor} do benefício \textemdash{} uma escolha de política
\textemdash{}, não por uma carga de casos em expansão.
...
regressão com efeitos fixos de UF ($R^2=0{,}71$). Parte do $-0{,}66$ bruto
reflete, portanto, o contraste \emph{entre} estados (Norte/Nordeste vs.\
Sul/Sudeste), mas uma parcela substancial e robusta sobrevive
\emph{intra}-estado. Trata-se, ainda assim, de uma \emph{associação}
estrutural, não de um efeito causal identificado: confundidores como o
nível de desenvolvimento local e a composição setorial não são isolados, e
um $n$ grande aumenta a precisão amostral sem eliminar esse viés. A
identificação causal do efeito do emprego sobre a demanda por transferência
exigiria variação exógena \textemdash{} um instrumento \emph{shift-share},
uma descontinuidade de elegibilidade ou um \emph{event study} sobre choques
locais de emprego \textemdash{}, agenda que registramos na
