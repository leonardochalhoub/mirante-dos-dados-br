...
(CNES/DATASUS), processados pelo pipeline aberto \textit{Mirante
dos Dados} em arquitetura medallion (bronze\textrightarrow silver
...
\textit{Mirante dos Dados} medallion pipeline
...
Bolsa Família do projeto \textit{Mirante dos Dados} para
...
versão estendida do programa de pesquisa Mirante (consultar
...
\ref{fig:choro-pd}) adotam o \textit{colormap} \texttt{cividis\_r}
...
Mirante (maior valor $\to$ cor mais escura), mas a interpretação
do leitor deve ser informada pela legenda explícita e pela
discussão substantiva do contexto, não pelo gradiente em si. A
agenda Mirante para versões interativas em \textit{web} contempla
...
por UF $\times$ prevalência por faixa). RM por UF do gold Mirante
2025. Valores em $N_{\text{UF}}$ correspondem a número médio de
...
contribuição metodológica do projeto Mirante \textemdash{} que
