...
pró-rico; o índice de necessidade variou cerca de nove vezes entre os
...
Databricks, com governança e armazenamento pelo Unity Catalog
(\textit{Volumes}); a orquestração é declarada como código por meio de
\textit{Databricks Asset Bundles}, em que um \textit{job} em cascata
encadeia \texttt{ingestão $\rightarrow$ bronze $\rightarrow$ silver
$\rightarrow$ gold $\rightarrow$ export}, com dependências explícitas
que garantem a ordem de materialização. O código é distribuído sob
licença aberta e pode ser reproduzido por terceiros na edição gratuita
do Databricks, sem custo marginal.
...
mediana da OCDE 2021 ($\approx$17/Mhab).}
\end{figure}
...
cerca de nove vezes maior em Roraima (1.435 pacientes estimados por
aparelho) e Acre (1.318) do que no Distrito Federal (152) ou no Rio de
Janeiro (208). \textit{A magnitude dessa discrepância indica que a
subdotação de algumas UF não é incremental, mas estrutural: a pressão
relativa nas regiões mais desassistidas chega a ser nove vezes a do
Distrito Federal.}
...
contraposta à maior pressão de necessidade nas UF amazônicas,
...
até nove vezes maior nas regiões mais desassistidas. Os resultados
