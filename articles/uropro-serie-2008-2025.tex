...
%
% Este Working Paper é um seguimento de CHALHOUB & DA SILVA (2026,
% Working Paper n. 3), que por sua vez derivou da especialização de
% TATIELI (2022). O escopo aqui é mais amplo (2008–2025), foca em
% dinâmicas longitudinais e incorpora a correção da pipeline silver
% (bug de _ingest_ts identificado e resolvido em abril/2026).
% =====================================================================
...
\begin{titlepage}
  \begin{center}
    ...
    {\small\textit{Este artigo é o quarto da série de Working Papers do
     Mirante dos Dados. Continua e estende DA SILVA \& CHALHOUB
     (2026c, Working Paper n. 3), que por sua vez se assenta sobre
     a especialização em Enfermagem de TATIELI (2022). O recorte
...
habitantes; PE, AL e RR ficam abaixo de 0{,}75. O cruzamento com
agregados estaduais do Bolsa Família (Mirante WP n. 2) e das
Emendas Parlamentares (Mirante WP n. 1) revela que essa
desigualdade segue de perto o gradiente de pobreza estrutural
(correlação $\rho \approx -0{,}68$ entre cobertura PBF e
AIH/100k) e não é compensada por fluxo fiscal político: emendas
\textit{per capita} são, contraintuitivamente, \textit{maiores}
nas UFs de menor acesso. (iv)~A mortalidade hospitalar é
...
comparável à abdominal. Como mostraram TATIELI (2022) e DA SILVA
\& CHALHOUB (2026c), o SUS de fato consolidou essa preferência: a
razão vaginal:abdominal observada na série SIH-AIH-RD entre 2015 e
2020 estabilizou-se em torno de 7:1, alinhada à literatura.
...
\textit{os números aqui apresentados são diferentes dos números
publicados no Working Paper n. 3, e a diferença é causada por uma
correção de engenharia, não por revisão de fonte.}
...
reconstituição. Comparações com o Working Paper n. 3 ficam
prejudicadas em magnitudes absolutas, mas \textit{não} nas
direções de tendência \textemdash{} o que é um achado
...
ligeiramente maior do que o reportado no Working Paper n. 3
(que utilizou dados parciais por força do bug descrito na
...
\subsection{Acesso, pobreza e fluxo fiscal: cruzamento com
Bolsa Família e Emendas Parlamentares}\label{sub:cruzamento}

A pergunta natural diante da desigualdade descrita acima é:
\textit{ela acompanha outras dimensões de desigualdade
estaduais, ou é específica da rede de saúde?} Aproveitamos a
existência, na mesma plataforma Mirante dos Dados, de duas
outras verticais com agregados estaduais comparáveis
\textemdash{} o Programa Bolsa Família (PBF) e a execução de
Emendas Parlamentares \textemdash{} para um cruzamento simples
mas iluminador.

A Tabela~\ref{tab:cruzamento} apresenta, por UF e em média
anual 2024\textendash 2025: (i) a taxa de cirurgias vaginais
por 100\,mil habitantes (UroPro), (ii) o percentual da
população beneficiária do PBF (\textit{proxy} de pobreza
estrutural), e (iii) a execução de emendas parlamentares por
habitante em \rs 2021 (\textit{proxy} de fluxo fiscal
político).

\begin{table}[H]
\centering
\caption{Cruzamento entre acesso à cirurgia, pobreza e fluxo
fiscal por UF. Médias anuais 2024\textendash 2025. Fontes:
SIH-AIH-RD (DATASUS), CGU/Portal da Transparência (PBF e
Emendas), IBGE/SIDRA, BCB (IPCA).}
...
A leitura da tabela rende três observações fortes.

\textbf{(i) Acesso e pobreza estrutural são fortemente
correlacionados, em sentido inverso.} Os cinco estados com
maior acesso à cirurgia (SC, PR, GO, MS, SP) têm, todos,
percentuais de população em PBF entre 3{,}4 e 7{,}9\,\%. Os
cinco com menor acesso (AL, RR, PA, AP, PE) operam com
percentuais entre 12{,}8 e 18{,}1\,\%. A correlação de
Pearson entre as duas séries, calculada sobre os 27 pontos,
é de aproximadamente \textendash 0{,}68 \textemdash{} alta
para qualquer padrão de dado público brasileiro. \textit{Onde
há mais pobreza estrutural, há sistematicamente menor acesso
à cirurgia uroginecológica.} Esse não é um achado original
\textemdash{} a desigualdade de saúde no Brasil é fartamente
documentada \textemdash{} mas é uma quantificação direta sobre
um procedimento clínico específico, não sobre indicadores
agregados.

\textbf{(ii) Emendas parlamentares per capita não compensam o
gap de acesso.} A correlação entre emendas/habitante e
AIH/100k é, surpreendentemente, \textit{negativa}: os estados
que recebem mais recursos via emendas \textit{per capita} são
justamente os de menor acesso (AP recebe \rs 608/hab. e tem
0{,}74 AIH/100k; SP recebe \rs 44/hab. e tem 6{,}04). A leitura
descritiva mais charitable é que o sistema de emendas funciona
em parte como mecanismo de \textit{compensação fiscal} (mais
recursos para estados com maior fragilidade orçamentária
estrutural), mas o volume per capita extra \textit{não se
traduz} em ampliação visível de oferta cirúrgica \textemdash{}
pelo menos não para este procedimento específico. As emendas
parlamentares são, em média, alocadas em obras e custeio
genérico, não diretamente em capacitação de equipes
cirúrgicas especializadas.

\textbf{(iii) A correlação inversa não é monotônica
\textemdash{} TO, AC e MA são exceções relevantes.} Tocantins
tem 11\,\% da população em PBF \textit{e} 4{,}98 AIH/100k
(perto da média nacional); Acre, com 16\,\%, atinge 4{,}30;
Maranhão, com 19\,\%, registra 4{,}12. Essas três UFs operam
acesso surpreendentemente alto dada sua pobreza estrutural, o
que sugere que a barreira não é apenas econômica: existem
configurações específicas de rede pública que conseguem
\textit{driblar} o gradiente de pobreza. Investigar o que TO,
AC e MA fazem de diferente é uma agenda óbvia para política
pública de saúde, e está fora do escopo descritivo deste
trabalho.

A leitura agregada do cruzamento é sóbria: a desigualdade
territorial documentada na Seção~\ref{sub:geografia} \textit{não
é uma anomalia da rede de saúde}; é uma manifestação local de
uma desigualdade brasileira mais ampla, em que pobreza
estrutural e acesso a serviços especializados andam juntos. A
existência de transferências sociais (PBF) reduz a privação
de renda mas não cria, por si, capacidade hospitalar; a
existência de fluxo fiscal político (Emendas) sobe os números
\textit{per capita} de gasto sem necessariamente translacionar
em oferta de procedimento. \textit{Acesso à cirurgia
uroginecológica é função da rede instalada, não do dinheiro
disponível} \textemdash{} é a constatação que sintetiza a
evidência cruzada.
...
deste Working Paper \textit{não coincidem} com as tabelas
publicadas no Working Paper n. 3 (DA SILVA \& CHALHOUB, 2026c).
A diferença, contudo, não é pequena: na tabela de volumes por
UF, por exemplo, mais de metade dos estados aparecia com volume
zero em vários anos no WP n. 3 \textemdash{} um padrão que era,
afinal, artefato do filtro \texttt{\_ingest\_ts} descrito na
...
Urinária no Sistema Único de Saúde (2015\textendash 2020):
volumes, despesa, permanência e distribuição geográfica por via
de acesso. \textbf{Mirante dos Dados \textemdash{} Working
Paper n. 3}, abr. 2026c.\par\smallskip
...
\textemdash{} Working Paper n. 4}, v. 1.0, abr. 2026.
Disponível em:
\url{https://leonardochalhoub.github.io/mirante-dos-dados-br/}.

\vspace{0.5cm}

\noindent\textbf{Reconhecimento:} este Working Paper é continuação
direta de DA SILVA \& CHALHOUB (2026c, Working Paper n. 3), que
por sua vez teve como base o trabalho de especialização de
TATIELI (2022). A presente versão amplia o recorte temporal,
incorpora a correção da camada \textit{silver} (\textit{commit}
