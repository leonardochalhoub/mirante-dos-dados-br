...
export default function Equipamentos() {
  ...
  useEffect(() => {
    loadGold('gold_equipamentos_estados_ano.json')
      .then((all) => {
        // Silver fix 6f4e1ca + gold sync 4d47d37/08df2b2 já estão em produção:
        // total_avg = soma de GREATEST(qt_sus, qt_priv) por (CNES, mês) e
        // overlap dual-flag observado = 0,0 % (vide WP #6 §sub:dedup). Confiamos
        // no total_avg do gold direto — não há mais double-count a remover.
  ...
  return (
    <>
      ...
      <details className="no-print" style={{
        ...
        de modo que o <code>total_avg</code> publicado é o número físico canônico.
